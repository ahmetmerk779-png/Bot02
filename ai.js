...
function startBot(config) {
  ...
  bot.on('spawn', () => {
    logChat('[SİSTEM] Dünyaya adım atıldı. Velocity koruması için 8 saniye bekleniyor...');
  ...
  bot.on('forcedMove', () => {
    ...
    if (bot) {
        ...
        logChat('[SİSTEM] Işınlanma koruması: Fizik 3s kilitlendi.');
        setTimeout(() => {
            if (bot) {
  ...
  // 🛡️ AKILLI CHAT FİLTRESİ (Sadece ada göre veya özel mesajla tetiklenme)
  bot.on('chat', async (username, message) => {
    if (!username || username === bot.username || state.isQueueing) return;

    const botNameLower = bot.username.toLowerCase();
    const isNamed = message.toLowerCase().includes(botNameLower);

    // Sadece adı geçtiğinde yapay zekayı uyandırıyoruz
    if (!isNamed) return;
  ...
  // 💬 ÖZEL MESAJ (WHISPER) DİNLEYİCİSİ
